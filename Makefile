...
package-deb-doc:clean-deb
	...
	cp ./src/sonos package/debian/opt/thingsplex/sonos
	cp VERSION package/debian/opt/thingsplex/sonos
...
package-docker-amd:build-go-amd
	cp ./src/sonos package/docker/service
	cd ./package/docker;docker build -t sonos .

deb-arm : clean configure-arm build-go-arm package-deb-doc
	@echo "Building Thingsplex ARM package"
	mv package/debian.deb package/build/sonos_$(version)_armhf.deb
...
upload :
	...
	scp package/build/sonos_$(version)_armhf.deb $(remote_host):~/

remote-install : upload
	@echo "Uploading and installing the package on remote host"
	ssh -t $(remote_host) "sudo dpkg -i sonos_$(version)_armhf.deb"
